        ...
        <div className="w-full max-w-md">
          <div className={`rounded-xl shadow-lg p-8 ${isDark ? 'bg-slate-800' : 'bg-white'}`}>
            ...
            <div className="my-6 flex items-center">
              <div className={`flex-1 border-t ${isDark ? 'border-slate-600' : 'border-gray-300'}`}></div>
              <span className={`px-4 text-sm ${isDark ? 'text-slate-400' : 'text-gray-500'}`}>или</span>
              <div className={`flex-1 border-t ${isDark ? 'border-slate-600' : 'border-gray-300'}`}></div>
            </div>

            <button
              onClick={handleGoogleLogin}
              className={`w-full py-3 border font-semibold rounded-lg transition-colors flex items-center justify-center space-x-2 ${
                isDark 
